...
export class CalcSpanService {
  ...
  calcSpan(input: range, output: range, inputPrim: boolean,  val = '', pointCount = 10,): [Array<point>, point] {
    ...
    const mPri: number = primaryRng.urv - primaryRng.lrv; // Slope for primary range 
    const mSec: number = secondaryRng.urv - secondaryRng.lrv; // Slope for secondary range
  ...
  private calcVal(primaryRng: range, secondaryRng: range, val: string, mPri: number, mSec: number): point {
    const valFl: number = parseFloat(val);
    let valPoint: point; 
  ...
  private interpolateSecondaryRng(range: range, pointCount: number): Array<number> {
    const rngStart: number = range.lrv;
    const rngEnd: number = range.urv;
    const secondaryPts: Array<number> = [];
